...
class Program
    {
        static int[] SelectionSortLoop(int[] intArgs)
        {
            for (int n = 0; n < intArgs.Length; n++)
            {
                for (int i = n + 1; i < intArgs.Length; i++)
                {
                    if (intArgs[i] < intArgs[n])
                    {
                        int temp = intArgs[i];
                        intArgs[i] = intArgs[n];
                        intArgs[n] = temp;
        ...
        {
            int[] intArgs = { 2, 4, 6, 1, 4, 8, 9 };
            intArgs = SelectionSortLoop(intArgs);
            foreach (int i in intArgs)
            {
                Console.Write("{0},", i);
            }
            Console.ReadLine();
...
合并排序(merge sort) 算法的时间复杂度为 $O(n\log_{2}n)$ 。
